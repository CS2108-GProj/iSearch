...
center_path = os.path.join(cur_path, 'sift_kmeans.txt')
...
def test():
    full_path = 'lena.png'
    #feat = sift(full_path)
    #print 'sift', len(feat), len(feat[0]), len(feat[1])
    feat = sift2(full_path)
    print 'sift2', len(feat), len(feat[0]), len(feat[1])
    #histo = sift_histo(full_path)
    #print histo
    sift_lsh = sift_lsh_list(full_path)
    print len(sift_lsh)
